...
# Linode API token
TOKEN=
# Linode Label we're looking for
LABEL=
...
jq -r '.data[] | select(.label | startswith("$LABEL")) | .ipv4[0], .ipv6 // empty' > IPs.txt
...
# Step 5: Extract allowed IPs from the response and format output
